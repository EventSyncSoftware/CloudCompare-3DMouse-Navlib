...
//! Legacy 3DxWare SDK backend for 3D mouse handling (Windows only)
/** Uses the old 3DxWare C SDK (si.h, siapp.h, spwmath.h) to read raw device
    data via native event filtering. This backend reads raw motion/button data
    and emits signals for the manager to handle.
**/
class CCAPPCOMMON_LIB_API Mouse3DLegacy : public I3DMouseBackend
{
	Q_OBJECT

  public:
	//! Default constructor
	explicit Mouse3DLegacy(QObject* parent);
	//! Destructor
	~Mouse3DLegacy() override;

	// I3DMouseBackend interface
	bool connect(QWidget* mainWidget, QString appName) override;
	void disconnect() override;
	bool isConnected() const override;
	QString backendName() const override { return tr("Legacy 3DxWare SDK"); }
	bool drivesCameraDirectly() const override { return false; }
