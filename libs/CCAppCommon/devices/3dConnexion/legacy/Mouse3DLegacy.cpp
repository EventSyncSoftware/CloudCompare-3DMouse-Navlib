...
/** This file is derived from the Qt wrapper for 3dConnexion devices graciously shared by Dabid Dibben:
    http://www.codegardening.com/2011/02/using-3dconnexion-mouse-with-qt.html
**/

#include "Mouse3DLegacy.h"
...
// Unique instance
static Mouse3DLegacy* s_mouseInputInstance = nullptr;
...
Mouse3DLegacy::Mouse3DLegacy(QObject* parent)
...
bool Mouse3DLegacy::connect(QWidget* mainWidget, QString appName)
{
	if (!mainWidget)
	{
		assert(false);
...
bool Mouse3DLegacy::isConnected() const
{
	return m_siHandle != SI_NO_HANDLE;
}

bool Mouse3DLegacy::onSiEvent(void* siGetEventData)
{
...
void Mouse3DLegacy::move3d(std::vector<float>& motionData)
{
	Q_EMIT sigMove3d(motionData);
}

void Mouse3DLegacy::on3dmouseKeyDown(int virtualKeyCode)
{
	Q_EMIT sigOn3dmouseKeyDown(virtualKeyCode);
}

void Mouse3DLegacy::on3dmouseCMDKeyDown(int virtualCMDCode)
{
	Q_EMIT sigOn3dmouseCMDKeyDown(virtualCMDCode);
}

void Mouse3DLegacy::on3dmouseKeyUp(int virtualKeyCode)
{
	Q_EMIT sigOn3dmouseKeyUp(virtualKeyCode);
}

void Mouse3DLegacy::on3dmouseCMDKeyUp(int virtualCMDCode)
{
	Q_EMIT sigOn3dmouseCMDKeyUp(virtualCMDCode);
}

void Mouse3DLegacy::GetMatrix(const std::vector<float>& vec, ccGLMatrixd& mat)
{
	assert(vec.size() == 6);
...
void Mouse3DLegacy::Apply(const std::vector<float>& motionData, ccGLWindowInterface* win)
{
	    ...
	    || CCCoreLib::GreaterThanEpsilon(std::abs(vec[5])))
	{
		...
		if (!bubbleViewMode)
		{
			Mouse3DLegacy::GetMatrix(vec, rotMat);
